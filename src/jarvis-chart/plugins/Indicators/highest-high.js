...
export default {
  ...
  inside: (p, context, { distance }) => {
    ...
    const highestHigh = values.map((value, index, array) => {
      const sliceInstance = values.slice(Math.max(0, index - distance), index + 1);

      return sliceInstance.reduce((acc, value) => Math.max(acc, value.max), -Infinity);
    });

    highestHigh.forEach((value, index) => {
      if (index < 1) return;
      if (index < offset || index > offset + count) return;

      const prev = highestHigh[index - 1];
